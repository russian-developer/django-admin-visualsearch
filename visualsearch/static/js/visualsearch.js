...
(function() {
  ...
  VS.VisualSearch = function(options) {
    ...
    if (this.options.query != 'null' && this.options.query.length > 0) {
        var $query = JSON.parse(this.options.query);
        var facets = [];
        ...
        $query.map(function(el){
            var searchFacet = new VS.model.SearchFacet({
                ...
                app      : app,
                label    : el.label,
                object_pk: el.object_pk
            });
            facets.push(searchFacet);
        });
        this.searchQuery.reset(facets);
    }
    else {
...
VS.ui.SearchBox = Backbone.View.extend({
  ...
  setQuery : function(query) {
    ...
    VS.app.SearchParser.parse(this.app, query, this.app.searchQuery.models);
  },
  ...
  showFacetCategoryMenu : function(e) {
    ...
    var items = [
      {title: 'Account', onClick: _.bind(this.addFacet, this, 'account', '', 'Account')},
      {title: 'Project', onClick: _.bind(this.addFacet, this, 'project', '', 'Project')},
      {title: 'Filter', onClick: _.bind(this.addFacet, this, 'filter', '', 'Filter')},
      {title: 'Access', onClick: _.bind(this.addFacet, this, 'access', '', 'Access')}
    ];
...
VS.ui.SearchFacet = Backbone.View.extend({
  ...
  setupAutocomplete : function() {
    this.box.autocomplete({
      ...
      select    : _.bind(function(e, ui) {
        ...
        this.set(ui.item.label, ui.item.value);
  ...
  set : function(value, pk) {
    if (!value || !pk) return;
    this.model.set({'value': value, object_pk: pk});
  },
  ...
  setupAutocomplete : function() {
    this.box.autocomplete({
      ...
      select    : _.bind(function(e, ui) {
        ...
        this.app.searchBox.addFacet(ui.item instanceof String ? ui.item : ui.item.value, '', position, ui.item instanceof String ? '' : ui.item.label);
        return false;
      }, this)
    });
...
VS.app.SearchParser = {
  ...
  parse : function(instance, query, models) {
    var searchFacets = this._extractAllFacets(instance, query, models);
    instance.searchQuery.reset(searchFacets);
    return searchFacets;
  },

  // Walks the query and extracts facets, categories, and free text.
  _extractAllFacets : function(instance, query, models) {
    var facets = [];
    var originalQuery = query;
    while (query) {
      var category, value, label;
      originalQuery = query;
      ...
      if (category && value) {
          var label = ''
          var model = models.filter(function(el){return el.get('category')==category})
          if (model.length > 0) {
              label = model[0].get('label')
          }
          var searchFacet = new VS.model.SearchFacet({
            category : category,
            ...
            app      : instance,
            label    : label,
            object_pk: model[0].get('object_pk')
          });
          facets.push(searchFacet);
      }
